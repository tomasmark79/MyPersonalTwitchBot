...
Socket::Socket(void)
    : mConnected(false), mInvalid(true), mReadReady(false),
      mReadyReadyOOB(false), mWriteReady(false), mTimedout(false), mSocket(-1),
      mReceivedBytes(0)
{
    // empty
}
...
int Socket::bind(const char *port, const char *ip)
{
    ...
    for (addrinfo *p = res; p != NULL; p = p->ai_next)
    {
        ...
        {
            cerr << "Unable to bind " << errno << " " << strerror(errno)
                 << endl;
            ::close(mSocket);
            mSocket = -1;
            continue;
...
int Socket::receive(char *buffer, int bufferLength, int timeout)
{
    ...
    do
    {
        ...
        if (mReadReady || mReadyReadyOOB)
        {
            ...
            int ret = ::recv(
                mSocket, buffer + received, bufferLength - received, flags
            );
