...
MyTwitchBot::MyTwitchBot()
{
    std::cout << "--- MyTwitchBot v." << MYTWITCHBOT_VERSION
    ...
    auto pointAdderLambda = [](Address &pointAdder,
                               Address &commandParser) -> void
    {
    ...
    auto commandParserLambda = [](CommandSet &mCommands,
                                  Address    &commandParser,
                                  Address    &pointAdder,
                                  Address    &transceiver) -> void
    {
        ...
        while (command != "SHUTDOWN")
        {
            ...
            cout << "State='" << state << "' Prefix='" << prefix
    ...
    auto transceiverLambda = [](Address &commandParser,
                                Address &transceiver,
                                Address &pointAdder,
                                Address &apiAddress) -> void
    {
        ...
        while (twitchConnection.connected())
        {
                ...
                throttle.check(messagesPerTick))
            {
                ...
                {
                    cout << "Sending message: '" << ptr << "' to twitch."
                         << endl;
                }
                ...
                do
                {
                    int sent = twitchConnection.send(
                        ptr + sentBytes, size - sentBytes, 100
                    );

                    if (sent < 0)
                    {
                        cout << "Something bad happened while sending message"
                             << endl;
                        break;
                    }
            ...
            do
            {
                ...
                if (!incompleteMessage.empty() &&
                    incompleteMessage.back() == '\r')
                {
                    incompleteMessage = incompleteMessage.substr(
                        0, incompleteMessage.size() - 1
                    );
             ...
             std::thread threadTimer(
                 [](Address commandParser, Address transceiver) -> void
                 {
                     while (!stopTimerThread.load())
                     {
                         ...
                         Message msg(
                             "PRIVMSG #digitalspacedotname : " + randomEmoji +
                             "\r\n"
                         );
    ...
    mCommands.registerCommand(
        {"!time",
             ...
             const std::vector<std::string> &params) -> void
         {
             ...
             Message     msg(
                 "PRIVMSG #digitalspacedotname : " + strTime + " " + "\r\n"
             );
             msg.send(commandParser, transceiver);
         }}
    );
    ...
    mCommands.registerCommand(
        {"!points",
         "return the number of points you have accrued.",
         [&](const std::string              &user,
             const std::string              &channel,
             const std::vector<std::string> &params) -> void
         {
    ...
    threads.push_back(std::thread(
        pointAdderLambda, std::ref(pointAdder), std::ref(commandParser)
    ));

    threads.push_back(std::thread(
        apiServerLambda, std::ref(transceiver), std::ref(apiAddress)
    ));
...
MyTwitchBot::~MyTwitchBot()
{
    std::cout << "--- MyTwitchBot uninstantiated ---" << std::endl;
}
